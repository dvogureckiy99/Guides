...
...
```
- Для точного определения размера используемого типа используется системный заголовочный файл `stdint.h`, который имеет определения типов `int8_t`, `uint32_t` и т.д. Как видно, такого рода типы определяют хранимый размер (число соответствует размеру в битах), в сравнении со стандартным типом `int`, который у компилятора `avr-gcc` (для 8-ми битных AVR) имеет размер 2 байта, а у стандартного `gcc` - 4 байта, что усложняет переносимость и понимание программы.
